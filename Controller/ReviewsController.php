...
class ReviewsController extends AppController
{
    ...
    public function isAuthorized($user)
    {
        ...
        if (in_array($this->action, ['edit', 'delete']) && isset($this->request->params['pass'][1])) {
    ...
    public function index($recipeId = null)
    {
        ...
        $options = ['conditions' => ['Recipe.'.$this->Recipe->primaryKey => $recipeId],
                'contain'        => [], ];
        $this->set('recipe', $this->Recipe->find('first', $options));
        $this->Review->recursive = 0;
        $this->Paginator->settings = $this->paginate;
        $this->set('reviews', $this->Paginator->paginate('Review', ['Review.recipe_id =' => $recipeId]));
