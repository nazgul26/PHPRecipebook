...
class MealPlansController extends AppController
{
    ...
    public function beforeFilter()
    {
        ...
        $this->filterConditions = ['MealPlan.user_id' => $this->Auth->user('id')];
    }
    ...
    public function isAuthorized($user)
    {
        ...
        if (isset($this->request->params['pass'][0]) && $this->request->params['pass'][0] != 'undefined'
                && in_array($this->action, ['edit', 'delete'])) {
    ...
    public function edit($id = null, $mealDate = null)
    {
        ...
        if ($this->request->is(['post', 'put'])) {
            ...
            if ($allSuccessful) {
                $this->Session->setFlash(__('The meal plan has been saved.'), 'success', ['event' => 'saved.meal']);
            ...
            $options = ['conditions' => ['MealPlan.'.$this->MealPlan->primaryKey => $id]];
    ...
    private function getMeals($weekList)
    {
        ...
        return $this->MealPlan->find('all', [
            ...
            'conditions' => array_merge($this->filterConditions, ['MealPlan.mealday BETWEEN ? AND ?' => [$start, $end]]),
        ]);
    }
}
