...
class MealMasterComponent extends Component
{
    ...
    public $currentIngredient = null;
    public $ingredientList = null;
    public $curRecipeString = null;

    /*
     * Define some reg ex parts to look for
     */

    public $rxpHead = '/^(M{5,5}|-{5,5}).*Meal-Master/';
    public $rxpEnd = '/^(M{20,200}|-{20,200}|-{5})/';
    public $rxpTitle = '/^ *Title: .+/';
    public $rxpCat = '/^ *Categories: .+/';
    public $rxpYield = '/^ *(Yield:|Servings:) +[1-9][0-9]*/';
    public $rxpWhitespaceOnly = '/^\\s+$/';
    // example group line: MMMMM----------------SAUCE-----------------
    public $rxpGroup = '/^(MMMMM|-----).*(-)+$/';
    public $rxpSeparator = '/^(MMMMM|-----)/';
    ...
    public function parseDataFileImpl($fp)
    {
        /* start */
    ...
    if (empty($this->importRecipes)) {
        echo "File does not contain any Meal-Master recipe(s).<br />\n";
 ...
 // readAllRecipes();
    ...
    public function readRecipe()
    {
        ...
        if (!empty($this->currentRecipe['name'])) {
            $this->importRecipes[] = ['Recipe' => $this->currentRecipe, 'Ingredient' => $this->ingredientList];
        }
 ...
 // readRecipe
    ...
    public function readRecipeHeader()
    {
    ...
    $done = false;
        ...
        } elseif (preg_match($this->rxpCat, $this->curRecipeString[0])) {
        ...
        } elseif (preg_match($this->rxpYield, $this->curRecipeString[0])) {
 ...
 // readRecipeHeader()
     ...
     */
    public function readRecipeBody()
    {
     ...
     */
    public function readRecipeIngredients()
    {
 ...
 // readRecipeIngredients()
    ...
    public function readIngredientLine($line)
    {
        $contLastLine = false;
 ...
 // bool readIngredientLine
    ...
    public function mightBelong2Ingredients($line)
    {
 ...
 // mightBelong2Ingredients()
    ...
    public function readRecipeSteps()
    {
        $skipLines = true;
 ...
 // readRecipeSteps()
    ...
    $list = split(' ', $cat);
        ...
        } elseif ($key = $this->containsValue($this->courses, $item)) {
            $this->currentRecipe['course_id'] = $key;
                //print "Got course ID: " . $this->courses[$key] . "<br/>";
        } elseif ($key = $this->containsValue($this->ethnicities, $item)) {
 ...
 // addCategories
     ...
     */
    public function containsValue($arr, $val)
    {
        foreach ($arr as $k => $v) {
            if (trim(strtolower($v)) == trim(strtolower($val))) {
                return $k;
            }
        }
